...
## 🤖 3. AGENT REPOSITORY INDEX

### 🏛️ T0: Core Orchestrators (Always Active)
- **Master Guide**: High-level strategy, orchestration, cross-project sync, and global memory management.
- **Healing Bot v2**: Predictive structural monitoring and autonomous remediation via `/dev fix`.
- **Swarm Router v3**: Routes tasks and resolves conflicts via multi-agent consensus (≥2/3 agreement).
- **Recursive Engine**: Analyzes session friction to convert corrections into permanent skill manifests.
- **Chaos Validator**: Injects controlled stressors to verify isolation boundaries and recovery.
- **Repository Agent**: Guardian of `/git auto` protocols, silent versioning, and industrial traceability.

### 🔬 T1: Specialized Sub-Agents (Dynamic Swarm)
- **Spec Architect**: Builds and validates `spec.md` + `spec.yaml` per the SDD Blueprint schema.
- **Contract Guardian**: Enforces `api-contract.yaml` and test fixtures; blocks implementation on failure.
- **Regional Adapter**: Injects MENA adaptations (Arabic RTL, Law 151 compliance, local payments).
- **Integrity Auditor**: Runs path integrity, sovereignty checks, and the `/audit` suite.
- **CI Specialist**: Autonomous repair of CI/CD pipeline failures via `/git action fix`.
- **Security Auditor**: Predictive security patching and hardening via `/git security fix`.
- **Visualize Agent**: Generates industrial-grade visual assets via `/create image`.
- **Creator Agent**: Scaffolds and assembles high-fidelity content via `/create content`.
- **Scraper Agent**: High-performance data extraction and ingestion via `/content scrape`.
- **SEO Agent**: Manages keyword strategy, meta-data integrity, and visibility audits.

### 🛠️ T2: Composition & Governance
- **Factory Scaffolder**: Assembles new sovereign workspaces from curated library components.
- **Profile Auditor**: Ensures workspace profiles align with library schemas and compliance rules.
- **Memory Manager**: Manages session state and short-term repository context.
