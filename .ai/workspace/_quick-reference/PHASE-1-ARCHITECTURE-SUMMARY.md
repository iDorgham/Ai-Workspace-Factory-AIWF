...
└────────────────────────────┬────────────────────────────────┘
              ...
              ├─ _fallback-routing.md ─ Decision logic
              ├─ claude-adapter.md ──── Claude specific
              ├─ gemini-adapter.md ──── Gemini specific
...
| `claude-adapter.md` | Claude's specific implementation | 350 lines |
| `gemini-adapter.md` | Gemini's specific implementation | 350 lines |
| `_fallback-routing.md` | Tool selection + fallback decision tree | 400 lines |
| `.ai/commands-multi-tool.md` | Updated command router with tool ranks | 500 lines |

### Governance (Data Rules)
| File | Purpose | Size |
|------|---------|------|
| `.ai/data-ownership-multi-tool.md` | File ownership + multi-tool versioning rules | 500 lines |
| `PHASE-1-SETUP-GUIDE.md` | Activation instructions + testing checklist | 300 lines |
...
Example:
  ...
  ✅ Gemini → allowed (in commands-multi-tool.md)
...
- `/create blog-posts` still works (routes to Claude per commands-multi-tool.md)
...
3. Update `.ai/tool-adapters/_fallback-routing.md` with new rankings
...
**Scalability:** ✅  
- Adding new tool = 1 adapter file + 3 lines in commands-multi-tool.md
