...
**Authoritative Files:** `content/sovereign/reference/brand-voice/`, `voice-refinement.md`, `style-rules.md`  
...
guide-agent
  ...
  → brand-agent [owns entire brand voice pipeline]
    ...
    └─ [IF /refine]
       ...
       │   └─ Input: Recent .md files in content/, style-rules.md
       ...
       │   └─ Input: drift-report.json, existing style-rules.md
       │   └─ Logic: Merge new patterns, flag conflicting rules
       │   └─ Output: Updated style-rules.md {rules: [{name, pattern, examples, priority}]}
       │   └─ Validation: No rule conflicts, all rules tested on ≥3 samples
       │   └─ Writes: content/sovereign/reference/brand-voice/style-rules.md + glossary.md
    │
...
- [ ] **Documentation** (this file) embedded in `.ai/AGENT-CLARITY.md` for reference during execution
