...
def explain_routing_mode(command, tool_registry):
    ...
    command_routing = load_json(".ai/commands-multi-tool.md")
...
Once routing decides which tool to use, the appropriate adapter (copilot-adapter.md, codex-adapter.md, etc.) is loaded and executed.
