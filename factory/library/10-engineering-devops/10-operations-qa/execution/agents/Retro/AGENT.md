...
**Memory update:** Saving [N] lessons to .ai/memory/lessons-learned.md
...
After every retro, @RetroFacilitator writes to `.ai/memory/lessons-learned.md`:
...
Identify: which template caused the gap?
  → .ai/templates/feature-plan.md (missing RTL test requirement)
  → .ai/templates/sprint-plan.md (missing contract lock checkpoint)
...
### Updates directly:
```
.ai/memory/lessons-learned.md   → new lessons added
.ai/templates/[relevant].md     → template patches applied
.ai/plans/active/current-sprint.md → action items added to next sprint
