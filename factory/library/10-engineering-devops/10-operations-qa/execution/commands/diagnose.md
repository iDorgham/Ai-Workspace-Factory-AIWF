...
id: commands:execution/commands/diagnose
version: 10.0.0
...
```
Load: .ai/memory/error-patterns.md
...
Check: is this issue already a known pattern?
  ...
  NO MATCH:
    ...
    → Capture as EP-[next] to error-patterns.md after resolving
```
...
**Quick depth:**
1. Check `.ai/memory/error-patterns.md` — known pattern?
...
### Error Capture (to .ai/memory/error-patterns.md)
...
**Every security finding → EP-[id] in error-patterns.md.**
...
2. Cross-reference failures with error-patterns.md
...
| "Pattern not found" | No match in error-patterns.md | New issue — proceed with full diagnosis |
