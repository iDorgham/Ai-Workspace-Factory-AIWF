...
| File | Purpose |
|------|---------|
| **`runtime_state.md`** | Schema + YAML template for `@RuntimeOrchestrator` |
| **`runtime-capability-weights.json`** | Empty weights envelope for `/swarm optimize --auto` |

**Discovery:** `scripts/validate/runtime_state_guard.py` checks root `sos/runtime_state.md` if present **and** every **`.ai/plans/active/features/*/sos/runtime_state.md`**.

---

*See: `docs/workspace/reference/feature_plan_package_layout.md`*
