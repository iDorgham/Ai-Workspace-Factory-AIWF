...
3. Write ONLY tests for uncovered behaviors
   See: .ai/skills/incremental_build_strategy.md (test coverage gap detection)
...
@Debugger activated:
  ...
  Step 5: @ErrorDetective captures EP-[id] to error_patterns.md
```
...
@QA self-healing attempt:
  ...
  5. Log self-healing event to: .ai/plans/active/audit/self_healing.md
  6. @ErrorDetective: is this a known flaky pattern? → check error_patterns.md
...
@ErrorDetective writes to .ai/memory/error_patterns.md:
...
Recurrence check:
  2nd occurrence of same pattern? → @ErrorDetective promotes to anti_patterns.md
...
Every `/test` run is logged to `.ai/plans/active/audit/command-logs/YYYY_MM_DD.md`:
