...
SOURCE_AP    → matches an existing anti-pattern (AP-[id])
               → @ErrorDetective logs to error_patterns.md
               → if this is the 2nd occurrence: escalate to recurrence alert
               → if 3rd+ occurrence: CRITICAL — AP injection may be failing

SOURCE_NEW   → new violation pattern not yet in anti_patterns.md
               → @ErrorDetective logs to error_patterns.md as candidate
...
CAPTURE: if contract drift detected → EP-[id] in error_patterns.md (matches AP-030)
```
...
CAPTURE: every security violation → EP-[id] in error_patterns.md
```
...
CAPTURE: test failures → EP-[id] in error_patterns.md
...
2. LOG to .ai/memory/error_patterns.md:
   --- ERROR CAPTURE ---
   Date: [YYYY-MM-DD]
...
Every `/quality` run is logged to `.ai/plans/active/audit/command-logs/YYYY_MM_DD.md`:
...
[YYYY-MM-DD HH:MM] /quality all
  ...
  Gate 6 build: PASS
  Error capture: EP-001, EP-002, EP-003 written to error_patterns.md
