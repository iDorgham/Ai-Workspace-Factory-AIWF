...
id: commands:execution/commands/retro
version: 10.0.0
...
The mistake prevention system has four layers (see `.ai/skills/mistake-prevention-system.md`):
...
Without `/retro`, error patterns accumulate in `error-patterns.md` but never become hard rules. Agents never get smarter. The same mistakes recur indefinitely.
...
FROM @ErrorDetective (sprint error report):
  Total errors captured in error-patterns.md this sprint
  Error patterns by type, agent, domain
  Errors already promoted to anti-patterns.md
...
Entries in error-patterns.md this sprint (not yet [PROCESSED]):
...
--- AP WRITE ---
Target file: .ai/memory/anti-patterns.md
...
|-------|-------------|-----------|--------|
| AP-054 | Missing icon aria-label | pre-flight-checklist.md | Add to @Frontend Pre-Flight section |
| AP-054 | Missing icon aria-label | hallucination-containment.md | Add to HIGH RISK zones |
| AP-055 | Redis TTL not set | pre-flight-checklist.md | Add to @Backend Pre-Flight section |
| AP-001 update | Directional CSS (raised severity) | lesson-injection.md | Update filter threshold |
...
.ai/skills/pre-flight-checklist.md:
...
When an AP is recurring despite being in `anti-patterns.md` (effectiveness <90%), the rule must be written directly into the agent's definition file under a `## Hard Rules` section:
...
**Write new lessons to `.ai/memory/lessons-learned.md`:**
...
For every EP entry reviewed in Step 5:
  Mark as: [PROCESSED-Sprint-N] in error-patterns.md

Entries to archive (3+ sprints old, [PROCESSED]):
  Move to: .ai/memory/archive/error-patterns-sprint-[N].md
  Preserve summary row in: error-patterns.md recurring patterns tracker
```
...
Retirement candidates (no trigger in 6+ sprints):
  ...
  Move to: .ai/memory/archive/anti-patterns-retired.md
...
4. Update .ai/memory/project-context.md (velocity trend, error rate trend)
...
| File | Change | AP-ID |
|------|--------|-------|
| pre-flight-checklist.md | Added @Frontend icon aria check | AP-054 |
| pre-flight-checklist.md | Added @Backend Redis TTL check | AP-055 |
| hallucination-containment.md | Added aria pattern to HIGH RISK | AP-054 |
...
Step 3: Promote qualifying patterns to anti-patterns.md
...
4. Verify lessons-learned.md has Sprint [N] section
5. Verify anti-patterns.md has new AP entries from Sprint [N]
...
8. Update .ai/memory/project-context.md (velocity trend, error rate trend, AP count)
9. Tag processed EP entries: [PROCESSED-Sprint-N] in error-patterns.md
...
✅ Lessons preserved: .ai/memory/lessons-learned.md (Sprint [N] section)
✅ [N] new APs written: .ai/memory/anti-patterns.md
...
Sprint 1: @Frontend uses margin-left (AP-001 violation)
          ↓
          /quality catches it → EP-001 captured in error-patterns.md
          ↓
...
/retro start (Sprint 1 end):
  ...
  Step 6: @KnowledgeSynthesizer:
          → Raises AP-001 from HIGH to CRITICAL in anti-patterns.md
          → Adds Hard Rule to .ai/agents/frontend.md:
            "NEVER use ml-/mr- classes. ALWAYS use ms-/me-. Violation = CRITICAL."
          → Adds check to .ai/skills/pre-flight-checklist.md @Frontend section
          → Updates lesson-injection.md: AP-001 now CRITICAL priority in filter
          ↓
...
*Writes to: anti-patterns.md, error-patterns.md, lessons-learned.md, skill files, agent files, project-context.md*
