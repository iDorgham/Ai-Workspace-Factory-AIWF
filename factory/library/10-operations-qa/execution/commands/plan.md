...
id: commands:execution/commands/plan
version: 10.0.0
...
```yaml
runtime_enabled: false        # true → expect /swarm --live behavior + sos/runtime-state.md updates
...
| Field | Type | Purpose |
|-------|------|---------|
| `runtime_enabled` | boolean | If `true`, feature expects adaptive loop; initialize `sos/runtime-state.md` on first `/swarm run --live`. |
...
- **Optional legacy:** per-spec **`[spec]/sos/`** (e.g. `runtime-state.md`) only when runtime hooks are enabled — do **not** create a repo-root **`sos/`** unless explicitly required.
...
Full reference: **`docs/workspace/reference/feature-plan-package-layout.md`**.
