...
PASSIVE MEMORY LOADING (current baseline):
  Agent loads .ai/memory/lessons-learned.md
...
function injectLessons(task: TaskContext, agent: AgentId): Lesson[] {
  const allLessons = loadMemory([
    ...
    '.ai/memory/anti-patterns.md',    // high-priority rules
    '.ai/memory/error-patterns.md',   // recent errors
  ])
...
INJECT ALWAYS:
- CRITICAL anti-patterns (AP-[id] from .ai/memory/anti-patterns.md)
