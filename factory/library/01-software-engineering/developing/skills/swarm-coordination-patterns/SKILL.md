...
## SDD alignment
Default swarms read the active **phase/spec** under **`.ai/plans/active/features/[phase]/[spec]/`**: **`plan.md`** (AC + Data Shape) and, after SOS, **`prompt.md`**. Phase **`manifest.md`** schedules parallel tiers when multiple specs exist. See **`.ai/skills/sdd-spec-workflow.md`** before splitting work across agents.
...
@RetroFacilitator runs structured retro
    ...
    → Updates .ai/memory/lessons-learned.md
